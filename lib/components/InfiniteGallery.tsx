...
// Mirrors the original scattered skills arrangement. Cycles if more images.
const LAYOUT = [
  ...
  { x: -0.8, y: -1.1, rot: -0.05 },
  { x: 1.1, y: -0.7, rot: 0.08 },
]
...
export function InfiniteGallery({
  ...
  zSpacing = 6,
  planeSize = [2, 2],
  headings = ['Languages', 'Frameworks', 'Database', 'Version Control'],
  headingStops = [0, 4, 6, 9],
  vhPerImage = 75,
  className,
}: Props) {
  ...
  const adjustedPlaneSize: [number, number] = isMobile
    ? [planeSize[0] - 0.2, planeSize[1] - 0.2]
